...
namespace PoGoEncTool
{
    internal static class EncountersGO
    {
        ...
        private const int MaxSpeciesID_GO_HOME = (int)Genesect;
        ...
        public static IEnumerable<PogoPoke> CreateSeed()
        {
            return GetSlots(MaxSpeciesID_GO_HOME, DisallowedSpecies, ExtraSpecies);
        }

        private static IEnumerable<PogoPoke> GetSlots(int maxSpecies, HashSet<int> banlist, int[] extras)
        {
            const int start = 1;
            ...
            foreach (var specform in speciesList)
            {
                ...
                for (int f = 0; f < pi8.FormeCount; f++)
                {
                    ...
                    if (IsDisallowedDuplicateForm(species, f))
                        continue;
                    yield return GetSlot(species, f);
                }
            }
        }

        private static PogoPoke GetSlot(in int species, in int form)
        {
            ...
            if (result.Count == 0)
            {
                ...
                if (!UnavailableShiny.Contains(sf))
                    result.Add(new PogoEntry { Type = PogoType.Wild, Comment = "Initial: Shiny can exist in wild" });
            }
